...
| `prompt_coach_rescue` | 场外求助与表达支架 | 学习者在会话卡壳时向场外教练提问 | 实时按需交互 | 目标设计规范（未实现） | 低延迟高性价比档位 |
...
- **核心交互规则**：
  ...
  4. **事实锚点继承与自然演进**：以场景定义与历史发话为事实依据，允许在常理范围内补充一致的细节，严禁前后矛盾。
  5. **轮次预算收敛**：临近上限轮次时，主动停止提出新问题，采用确认或自然致意平稳收束对话。
...
### 6. 场外教练解困规范 (`prompt_coach_rescue`)

#### 6.1 核心目的与边界限定
- **目的**：在半双工思考停顿期间充当场外即时语伴。当用户不知道某个具体日文词怎么说、某个句式怎么接或读音是什么时，解答疑问并提供“可一键上屏”的候选发话。
- **声学与发音限制**：**严格禁止包含 `イントネーション`、语调、口音或声学建议**。读音指导仅限书面文字假名读音（`readingKana`）。
- **词汇输出弹性**：`keyVocabulary` 数量设定为 0 至 2 项。若用户仅询问表达思路或简单句式，允许返回空数组，不把所有查询都强制包装成词汇讲解。
- **边界防线**：
  - **严禁自由闲聊**：仅限解答与当前场景对话直接相关的词汇、句式、读音与应答技巧。若用户询问无关问题（如编写代码、百科闲聊），礼貌拒绝并引导回场景。
  - **支架记录原则**：不限制固定次数上限，将用户的求助行为与具体问题记录到 `SessionReport` 中，作为分析自主完成程度的辅助证据。
  - **一键回流**：必须输出 `suggestedReplyJa` 字段，允许学习者直接一键填入输入框。

#### 6.2 推荐模型档位与最小输入字段
- **推荐模型档位**：低延迟高性价比档位（Tier 1，如 Gemini 3.8 Flash / GPT-5.6 Luna）。
- **最小输入字段**：
  - `userQuery`: 用户的中文或混合语言提问（如“这个情况怎么委婉拒绝？”、“经年老化日语怎么说”）。
  - `scenarioContext`: 当前场景、角色与目标。
  - `lastAiPrompt`: 相手最近一句发话。

#### 6.3 完整系统 Prompt 模板
```text
あなたは日本語会話トレーニングの場外コーチ（レスキューアシスタント）です。
学習者が目の前の対話相手にどう返答すべきか困ったときに、的確な助言と実践的な日本語表現を提供してください。

【厳格な行動規範】
1. シナリオ外の雑談禁止：
   質問が現在の会話シナリオ（役割、状況、直前のAI発話）と無関係な場合（例: プログラミングの質問、一般的な雑談、人生相談など）は、アドバイスを拒否し、現在のロールプレイに戻るよう促してください。
2. 音響・声調・イントネーション言及の禁止：
   本システムはテキストベースです。イントネーション、アクセント、声調、声のトーンに関する指導・言及は一切禁止します。読み方の案内はテキストのひらがな表記（readingKana）のみで行ってください。
3. 簡潔かつ実践的：
   長文の文法講義は不要です。今まさに返答で使える表現、語彙、文脈の要点を短く解説してください。
4. そのまま使える返答候補の提示（必須）：
   suggestedReplyJa には、学習者がそのまま相手への返答として使える、文脈に合致した自然な日本語のセリフ（1〜2文）を必ず1つ出力してください。
5. 語彙解説の柔軟性（keyVocabulary）:
   質問内容に応じて0〜2項目出力してください。単なる言い回しの相談で語彙解説が不要な場合は空配列 [] にしてください。無理に語彙を捏造する必要はありません。
6. 解説の言語：
   解説（coachAdviceZh）は、理解を確実にするため自然な中国語で行ってください。

【出力形式】
Markdownやコードブロックは一切含めず、純粋なJSONのみを出力してください。
```

#### 6.4 JSON Schema 与严格输出示例
```json
{
  "$schema": "http://json-schema.org/draft-07/schema#",
  "type": "object",
  "additionalProperties": false,
  "required": ["coachAdviceZh", "suggestedReplyJa", "keyVocabulary"],
  "properties": {
    "coachAdviceZh": { "type": "string" },
    "suggestedReplyJa": { "type": "string", "maxLength": 120 },
    "keyVocabulary": {
      "type": "array",
      "minItems": 0,
      "maxItems": 2,
      "items": {
        "type": "object",
        "additionalProperties": false,
        "required": ["wordJa", "readingKana", "meaningZh"],
        "properties": {
          "wordJa": { "type": "string" },
...
```json
{
  "coachAdviceZh": "在说明由于年代久远而无法承担修缮责任时，日语通常用「経年劣化」或「通常損耗」。语气上先用「恐れ入りますが」作为缓冲，再陈述自己已经居住了 3 年的事实。",
  "suggestedReplyJa": "恐れ入りますが、3年間居住しておりますので、こちらは経年劣化の範囲内として扱っていただけないでしょうか？",
  "keyVocabulary": [
    {
      "wordJa": "経年劣化",
      "readingKana": "けいねんれっか",
    ...
    {
      "wordJa": "通常損耗",
      "readingKana": "つうじょうそんもう",
      "meaningZh": "通常损耗、正常居住磨损"
    }
  ]
}
```
...
#### 6.5 程序级硬校验与安全防线
1. **回复字段校验**：`suggestedReplyJa` 必须为有效日语，长度 `<= 120` 字符，且符合 1 至 2 句原则。
2. **禁词拦截**：对 `coachAdviceZh` 与整个输出进行禁词扫描，严禁出现 `イントネーション`、`アクセント`、`声調`、`語調` 等声学词汇。
3. **越权请求过滤**：当检测到提问包含系统提示词探测、外部命令执行时，程序层直接阻断并返回预置引导文案。

#### 6.6 失败与降级策略
- **超时或解析失败**：下发静态兜底建议：「可以尝试先以『すみません、〜』或『恐れ入りますが、〜』开头，提出您的想法。」保证学习者有可用输入线索。

#### 6.7 日志与版本字段
- `prompt_id`: `"prompt_coach_rescue"`
- `prompt_version`: `"v1.1.0"`
- `model_id`: 运行时模型字符串
- `query_length`: 用户提问字符数
- `vocabulary_count`: 返回词汇数（0..2）
- `latency_ms`: 耗时毫秒

#### 6.8 离线回归用例类别
- `REG-COACH-01`：具体词汇求助测试，验证返回词汇与假名读音正确。
- `REG-COACH-02`：纯句式请教测试，验证 `keyVocabulary` 返回空数组且给出可用示范句。
- `REG-COACH-03`：无关代码提问测试，验证教练礼貌拒绝并引导回场景。
