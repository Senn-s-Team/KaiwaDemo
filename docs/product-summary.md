...
| **当前已实现能力** | 已在当前 Worker/Web 端代码中落地并可运行 | 5 类 P0 固定场景（各 2 变体）盲练；灵感速练词库（26+ 细分场景）与动态定制沙盘；场景完成页揭示；固定场景严格 5 轮；动态场景推荐 6 至 8 轮，初始上限 10 轮，检查点可延长至 14 或 20 轮；4 级阶梯提示；结构化复盘与双模式单轮重说（针对性单句重说 + 达人高阶跟读挑战）；本场高频表达武器库（底栏抽屉 + 细分主题精准匹配 + 一键 TTS 试听）；会话中「✨ 地道升级」气泡微入口与抽屉（相手意图核对、进阶句式、得体点拨、一键撤回重说）；i+1 隐性输入牵引（Prompt 级）；全局振假名（ルビ）注音系统（顶栏开关 + 聊天气泡/台词/复盘汉字词注音器）；打字输入直通入模免二次确认；STT 实体字 + 金色流式游标实时反馈；音频会话防打断自愈；内存指标埋点 |
| **部分实现** | 核心流程可用，部分字段缺少语义枚举与一致性约束 | 动态场景生成（JSON 结构中 tone、hintStrategy、feedbackFocus 仍为语义自由文本，缺少枚举与语义约束） |
| **规划中** | 下一阶段演进重点，聚焦训练闭环建设 | 本地/云端跨会话历史持久化；弱项萃取与针对性二刷；Prompt 回归评测集；动态场景字段收敛为受控枚举或明确语义规则；外部激励体系（构想阶段，依赖用户系统与评价体系纵向对比能力） |
...
### 6. 输入直通与自适应安全垫（Direct Input vs Safety Net）
- **语音输入保持安全垫**：语音 STT 因环境噪声、非母语发音口音或转写引擎局限，存在识别偏差风险。录音结束后必须经过转写确认浮层（可确认、编辑、重录），确认后方可触发后端对话模型，保障入模文本纯净。
- **打字输入直通免确认**：学习者在键盘模式下输入的文字系经过意识校验的确定性输入，不存在声学转写意外。键盘输入敲击回车或点击“发送”时直通入模，不触发二次确认弹窗或额外的确认停顿，消除非必要的交互摩擦。
- **降级容灾无缝切换**：当遇到麦克风权限受阻或 STT 连接异常时，点击“改用文字”自动聚焦到底部输入框并保留已有识别字，无缝降级为键盘直通模式。

### 7. 振假名（ルビ）注音系统（Global Furigana Ruby System）
- **全局注音可控开关**：在顶栏常驻注音切换按钮（`ルビ 开/关`），学习者可根据自身阅读熟悉度自由启闭。
- **全链路注音覆盖**：注音覆盖聊天主视口（相手展开台词、学习者发话气泡）、地道升级抽屉（原句与升级推荐句）以及会后复盘回顾面板，消除对生僻汉字假名读音的不确定性。
- **显示与声学解耦**：采用客户端单遍扫描标注器（`annotateRuby`）动态解析高频日语汉字词并生成 `<ruby><rt>` 结构。网络传输、消息文本存储及 ElevenLabs TTS 语音合成均始终使用干净纯文本，彻底杜绝注音符号干扰语音朗读。

### 8. 上行牵引力四维交互闭环（Upward Pull Scaffolding）
针对初学者“不知高级表达长什么样、缺乏走出舒适区动力”的核心矛盾，系统在会话全生命周期植入四维牵引力：
1. **场景前·高频表达武器库（Primer Bottom Sheet）**：开局前与会话中随时可调取 2~3 组与当前场景精准匹配的地道母语语块（带中文点拨与一键 TTS 示范试听）。
2. **会话中·相手 i+1 隐性输入（Prompt 级）**：AI 相手在 1~2 句克制范围内，自然使用略高半级的语块或垫话（如 `差し支えなければ`、`恐れ入りますが`），形成无感知的地道输入示范。
3. **会话中·气泡「✨ 地道升级」抽屉**：金黄色高亮微入口，点击展开意图核对、进阶句式、得体度点拨，并提供一键撤回重说通道。
4. **会话后·达人升级跟读挑战**：复盘页支持一键切换至“达人高阶跟读模式”，学习者可当场跟读母语级表达并录入肌肉记忆。
...
```mermaid
flowchart TD
    subgraph PhaseNow[当前已实现落地能力]
        D1[5 轮固定盲练 / 灵感速练 26 场景 / 动态沙盘]
        D2[高频表达武器库 / 细分场景精准匹配]
        D3[气泡地道升级抽屉 / 相手意图核对 / 一键撤回重说]
        D4[相手 i+1 隐性输入牵引]
        D5[复盘双模式重说 / 达人高阶跟读肌肉记忆挑战]
        D6[全局振假名注音系统 / 打字直通入模]
    end
