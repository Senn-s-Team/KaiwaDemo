...
- 用户能在灵感生成和中文自定义场景之间做出选择。
- 场景准备阶段只展示开始训练所需的信息，不提前泄露相手的具体首句或完整答案。
- 音频播放、重听、查看帮助、录音、停止、确认、修改、重录、重试和退出都有明确状态与结果。
- 用户能够区分听不懂和不知道怎么说，并进入对应支架。
- 会话时长和轮次可预期，相手不会无限扩展话题。
- 完成页信息克制，用户能理解沟通结果、一个听力发现、一个表达改进和一个重做任务。
...
相手每轮先以音频出现，台词默认折叠。用户正常路径以语音回应。文字用于确认转写，也用于设备、权限或网络不支持语音时完成当前会话。

### 6.6 支架按需出现，并逐步撤除

听力困难和表达困难分成两条支架路径。系统默认不摊开字幕、翻译和完整参考句。用户逐级获得帮助，重做时支架重新收起，让用户尝试减少依赖。
...
适用于用户想练习，但暂时说不出具体场景的时刻。

用户选择灵感生成后，系统根据核心用户画像生成一组简短候选。候选应覆盖不同沟通功能或生活领域，每个候选只展示：
...
用户选择一个候选后，系统直接生成正式场景。用户可以刷新候选，但刷新不会创建会话，也不会留下未完成训练记录。
...
内容领域可以自由变化，但每个正式场景仍然只训练一个核心沟通目标。

### 8.2 入口二：中文自定义场景

用户用中文描述想练的情境，例如沟通对象、想完成的事情、担心的条件或希望练习的语气。
...
- `speechAssistUsed`，仅由本轮是否存在 `displayed=true` 的 `speechAssistEvents` 推导，不由提示文案或请求成功推断。
- 必要的时间信息。
...
每轮相手发话先以自然语速音频出现。听力支架初始为 L0，表示不提供任何帮助。用户可以进入 L1 原速重听，重听不增加轮次；L3 之前不得显示日语台词，L4 之前不得显示中文意图概要。
...
- 听力支架缓存按相手消息保存，可序列化，并随当前标签页的单场 `sessionStorage` 恢复。缓存不进入提交给 LLM 的会话 history，history 仍只含消息的 `role` 与 `text`；缓存也不构成数据库、长期历史或跨设备同步。
...
聚合场景版本、会话事实、所有回合对、支架、转写、故障、沟通结果、复盘、重做和用户自评。每轮保留完整 `speechAssistEvents`，并从其中是否存在 `displayed=true` 推导 `speechAssistUsed`。报告还必须包含：
...
当前标签页把单场会话的场景、消息（包括按相手消息缓存的 `listeningScaffold`）、回合记录、当前回合、`turn` 与可编辑转写保存到浏览器 `sessionStorage`。刷新后可以恢复这些事实和最近的可操作位置，但这不是数据库、长期历史或跨设备持久化。
...
- 每轮表达支架、正式回应、`speechAssistEvents` 与从 `displayed=true` 推导的 `speechAssistUsed`。
- 依赖失败、重试、降级与显式恢复状态。
...
账号、同步、长期历史和个性化只能在明确的数据授权、删除、同步冲突和版本规则下接入。首版的本地会话标识和报告必须能够迁移，但不为未来身份体系提前收集数据。
...
- 听力支架与表达支架独立展开和记录，听力内容不会被记为表达帮助。
- 相手每轮一到两句，最多一个聚焦问题。
- 相手不跳出角色纠错，不编造已确认事实。
- 用户未确认转写前不会触发下一轮。
...
- 满足触发条件时 A 真实展示，非空 B 也真实展示；停止或继续发话后清除。每轮 `speechAssistUsed` 只由 `displayed=true` 的事件推导。
