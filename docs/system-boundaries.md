...
- 场景草拟和反馈/重做是 Worker 创建并查询的 Cloudflare Workflow durable task；普通会话回复、提示、听力支架、语音辅助和场景润色在 API 请求内完成。用户明确同意后，浏览器通过独立 outbox 向 D1 写入不含对话文本的验证事实；D1 不参与训练主链路。
...
| `/api/validation/batch` | POST | 以 shared 严格 Schema 校验同意式匿名技术验证批次，验证会话绑定的 telemetry token，并幂等写入 D1 | `telemetry` token；`VALIDATION_DB` |
...
- 只有本地同意状态为 `accepted` 且当前会话持有内存态 telemetry capability 时才会入队。在线、窗口重新获得焦点或新记录入队时尝试投递；发送器只接收与当前内存 session id 匹配的记录，旧会话残留记录会被丢弃而不会借用新 token。失败采用有界指数退避，所有失败都不得改变训练状态。
- 撤销同意会停止新记录、清空未发送 outbox，并删除本地匿名客户端 UUID。telemetry capability 不写入 sessionStorage、localStorage、完成复盘恢复记录或练习历史；刷新后不能恢复该页面内存凭据，也不会为旧会话继续生成遥测。

### 4.5 D1：匿名技术验证事实

- `worker/validation-telemetry.ts` 在 `/api/validation/batch` 边界验证 Schema、token 会话绑定和七天事件时间窗，然后把随机客户端 UUID 经过服务端 HMAC 派生后写入 `VALIDATION_DB`；D1 不保存原始 UUID、IP 字段、User-Agent、对话、转写、场景描述、音频或自由文本评价。
...
- 明确同意后的匿名验证批次只携带固定枚举、计数、耗时和粗粒度环境分类。D1 保存服务端 HMAC 派生客户端标识，不保存对话、转写、音频、场景描述、完整 User-Agent、原始客户端 UUID 或应用级 IP 字段；平台网络日志仍服从 Cloudflare 配置。
...
6. 匿名遥测字段、同意/撤销、token 内存边界、D1 migration、索引和 retention cron 是否仍同构。
