...
- 角色、关系、唯一目标和“开场：你先说/对方先说”在准备信息中纵向排列，360px 下不做并列操作；不开设开场切换控件。
...
- user-opening 第 1 轮用轻量状态文案提示用户开场，不伪造相手消息气泡；该轮隐藏相手播放与 L0-L4 听力入口，保留表达帮助。
- 主操作始终位于底部拇指可达区域。
- 录音中当前转写必须可见，辅助建议可以收起。
- 确认稿 Bottom Sheet 保留一个明确的“怎么说”入口；打开表达帮助时先关闭确认稿，关闭帮助后从底部的“检查/修改回答内容”返回，不能叠加两个 Sheet。
- 右上角关闭确认稿只隐藏当前 Sheet，不因仍处于 `confirming_transcript` 阶段自动重开；原转写稿、修改内容和错误提示必须保留。
