...
10. **可选本机录音**：首页默认不保存用户录音。用户明确开启后，正式回合和完成页重做只在确认转写/第二稿后保存到独立浏览器 IndexedDB；重录、文字切换、取消、失败或离页会丢弃未确认录音。完成页可按回合播放，首页历史按实际可用性提供录音入口；只保留最新 20 个录音会话，删除场景会级联删除关联录音。录音 Blob 不进入报告、练习历史对象、遥测或网络请求，保存失败不阻断训练。
11. **单场恢复、数据导出与匿名技术验证**：当前会话的场景、消息、回合记录、当前回合、`turn`、可编辑转写及按相手消息保存的听力支架缓存在浏览器 `sessionStorage`。刷新后恢复到最近可操作状态；录音或网络请求等不可原样恢复的阶段安全降级到等待用户回应或确认转写，不自动重放请求。训练事实、重做记录与 `speechAssistEvents` 可主动导出为 JSON。技术验证数据在测试使用期间自动记录并通过独立浏览器 outbox 发送，仅包含固定形状的匿名操作、会话和性能事实；遥测 token 仅存于内存，失败不阻断训练。
...
测试使用期间会自动发送匿名技术验证数据。发送内容仅为固定形状的流程阶段、耗时、失败类别、重试、功能使用计数、粗粒度环境分类、匿名会话/客户端派生标识和结构化复盘事实；wire schema 版本为 2，并使用 `anonymousMetricsCollection: true` 表示该批次属于自动技术验证。不会发送或在 D1 保存原始音频、本机录音 Blob、转写或对话内容、场景描述、访问令牌、IP、User-Agent 或完整浏览器标识。用户明确开启的本机录音只在确认回答后写入独立浏览器 IndexedDB，不进入遥测或网络。浏览器 outbox 只保存待发送事实、随机会话 ID 和重试元数据；telemetry token 只存在内存。`VALIDATION_DB` 缺失、D1 写入失败、浏览器存储不可用或遥测上传失败都只会丢失对应验证数据，不得阻断训练。保留期和边界见 [PRIVACY.md](./PRIVACY.md)。
...
3. `sessionStorage` 只恢复当前浏览器标签页中的单场会话，不构成账号同步或跨设备恢复；可选用户录音使用独立 IndexedDB，只保留最新 20 个录音会话，并且仅在确认回答后保存。自动匿名技术验证另由独立 outbox 发送至 `VALIDATION_DB`，用户仍需主动点击“导出训练数据 JSON”保存报告。
4. 真实语音识别和合成受浏览器麦克风权限、音频采样率和网络延迟影响；本机录音另受 MediaRecorder、IndexedDB 和设备存储容量影响，其失败不阻断训练。
