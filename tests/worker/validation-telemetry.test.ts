...
async function makeBatch(records: ValidationBatch['records']): Promise<ValidationBatch> { return { schemaVersion: 1, batchId: crypto.randomUUID(), sessionId, telemetryToken: await signTelemetryToken(env, sessionId), anonymousClientId: clientId, anonymousMetricsConsent: true, sentAt: Date.now(), records } as ValidationBatch }
...
describe('validation telemetry schema', () => {
  it('rejects dialogue, text, and unknown fields', () => {
    ...
    expect(ValidationBatchSchema.safeParse({ schemaVersion: 1, batchId: crypto.randomUUID(), sessionId, telemetryToken: 'token', anonymousClientId: clientId, anonymousMetricsConsent: true, sentAt: Date.now(), records: [{ ...sessionSummary, scenarioId: '含中文', variantId: 'v', completionReason: 'turn_budget' }] }).success).toBe(false)
  })
  it('enforces consent, UUID identity, and 25-record bound', () => {
    const base = { schemaVersion: 1, batchId: crypto.randomUUID(), sessionId, telemetryToken: 'token', anonymousClientId: clientId, anonymousMetricsConsent: true, sentAt: Date.now(), records: [checkpoint(0)] }
    expect(ValidationBatchSchema.safeParse(base).success).toBe(true)
    expect(ValidationBatchSchema.safeParse({ ...base, anonymousMetricsConsent: false }).success).toBe(false)
