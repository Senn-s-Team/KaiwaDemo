...
describe('Home realtime STT', () => {
  ...
  it('离线取消中文语音输入，保留已写入 partial 并拒绝迟到结果', async () => {
    const stopResult = deferred<string>()
    mocks.stop.mockReturnValue(stopResult.promise)
    button(container, '中文语音输入').click()
    await flush()
    flushSync(() => mocks.handlers?.onPartial('我想剪短'))
    expect(textarea(container).value).toBe('预约理发\n我想剪短')
    button(container, '说完了').click()
    await flush()
    flushSync(() => root.render(createElement(Harness, { online: false })))
    await flush()
    await vi.waitFor(() => {
      expect(button(container, '中文语音输入')).toBeDefined()
      expect(button(container, '说完了')).toBeUndefined()
      expect(container.textContent).not.toContain('正在整理识别结果')
    }, { interval: 0 })
    expect(mocks.release).toHaveBeenCalled()
    expect(textarea(container).value).toBe('预约理发\n我想剪短')
    flushSync(() => mocks.handlers?.onPartial('迟到 partial'))
    stopResult.resolve('迟到 final')
    await flush()
    expect(textarea(container).value).toBe('预约理发\n我想剪短')
    expect(container.textContent).not.toContain('迟到')
  })
