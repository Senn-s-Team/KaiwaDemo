...
app-lifecycle.test.ts: 真实 App 挂载下录音前相手播放器释放、草稿/前后台生命周期、同意遥测 checkpoint、活跃/完成态撤销控件及提前退出原因，以及遥测凭据不进入会话或完成恢复序列化的回归契约
...
validation-consent.test.ts: 遥测同意三态、存储拒绝、撤销 UUID tombstone 与匿名本地客户端标识边界
validation-outbox.test.ts: sessionId 路由的无令牌 IndexedDB 队列、跨会话隔离、每会话 dedupe 与撤销会话 tombstone、退避投递、撤销清空与内容脱敏契约
