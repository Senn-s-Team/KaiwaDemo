 ...
 * [OUTPUT]: 锁定无凭据出站、重复 record 幂等、退避投递、撤销清空与内容严格脱敏
...
import { createValidationDedupeRegistry, createValidationRoundSnapshot, createValidationSequenceClock, createValidationSessionRegistry } from '../../src/lib/use-validation-telemetry'
...
describe('validation telemetry outbox', () => {
  ...
  it('clears queued telemetry on revocation', async () => { installDatabase(); await enqueueValidationRecord('batch', 'dyn_ses_current', 1, record); await clearValidationOutbox(); expect(await listValidationRecords()).toHaveLength(0) })
  ...
  it('keeps a revoked session ineligible while allowing a later session', () => {
    const sessions = createValidationSessionRegistry()
    sessions.revoke('dyn_ses_revoked')
    expect(sessions.allows('dyn_ses_revoked')).toBe(false)
    expect(sessions.allows('dyn_ses_later')).toBe(true)
  })
