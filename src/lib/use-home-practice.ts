...
export function useHomePractice({ online, activeSession, resetSession, setUiError }: { online: boolean; activeSession: boolean; resetSession(): void; setUiError(error: UiError | null): void }) {
  ...
  const [pendingClarification, setPendingClarification] = useState<{ questionZh: string; optionsZh: readonly string[] } | null>(null); const [readyScenarioData, setReadyScenarioData] = useState<{ scenario: DynamicScenarioData; scenarioToken: string; practiceToken?: string; previousAdvice?: PreviousAdvice } | null>(null)
  ...
  const preparePracticeAgain = useCallback(async (attempt: PracticeAttempt) => { if (!online || isDraftingScenario) return; resetSession(); setCustomInputZh(attempt.scenario.userGoal); restartAbort.current?.abort(); const controller = new AbortController(); restartAbort.current = controller; setIsRestartingPractice(true); try { const ready = await restartPractice(attempt.practiceToken, AbortSignal.any([controller.signal, AbortSignal.timeout(25_000)])); const improvement = attempt.feedback?.expressionImprovement; const previousAdvice = improvement ? { expressionImprovement: improvement, sourceSessionId: attempt.report.sessionId, sourceStartedAt: attempt.report.startedAt, viewed: false } : undefined; if (!controller.signal.aborted) setReadyScenarioData({ ...ready, previousAdvice }) } catch (error) { if (!controller.signal.aborted) { setUiError(toUiError(error)); setHistoryNotice('原场景暂时无法载入，可从最近练过中重试。') } } finally { if (restartAbort.current === controller) setIsRestartingPractice(false) } }, [isDraftingScenario, online, resetSession, setUiError])
  const markReadyAdviceViewed = useCallback(() => setReadyScenarioData((ready) => ready?.previousAdvice && !ready.previousAdvice.viewed ? { ...ready, previousAdvice: { ...ready.previousAdvice, viewed: true } } : ready), [])
