...
export function useHomePractice({ online, activeSession, resetSession, setUiError }: { online: boolean; activeSession: boolean; resetSession(): void; setUiError(error: UiError | null): void }) {
  ...
  const [pendingClarification, setPendingClarification] = useState<{ questionZh: string; optionsZh: readonly string[] } | null>(null); const [readyScenarioData, setReadyScenarioData] = useState<PreparedRestartData | null>(null)
  ...
  useEffect(() => { setReadyScenarioData(readPreparedRestart()) }, [])
  ...
  useEffect(() => { const recovered = draftRecovery.state; if (!recovered.request) return; clearPreparedRestart(); setCustomInputZh(recovered.request.inputZh); setClarifications(recovered.request.clarifications.map((item) => ({ questionZh: item.questionZh, answerZh: item.answerZh }))); if (recovered.status !== 'ready') return; if (recovered.result.status === 'needs_clarification') { setReadyScenarioData(null); setPendingClarification({ questionZh: recovered.result.questionZh, optionsZh: recovered.result.optionsZh }); return }; setPendingClarification(null); setReadyScenarioData({ scenario: recovered.result.scenario, scenarioToken: recovered.result.scenarioToken, practiceToken: recovered.result.practiceToken }) }, [draftRecovery.state])
  const isDraftingScenario = isRestartingPractice || draftRecovery.state.status === 'submitting' || draftRecovery.state.status === 'pending'
  const updateCustomInputZh = useCallback((value: SetStateAction<string>) => { clearPreparedRestart(); setReadyScenarioData(null); setCustomInputZh(value) }, [])
  const submitDraft = useCallback((prompt?: string, answers?: Array<{ questionZh: string; answerZh: string }>, force = false) => { const input = prompt ?? customInputZh; if (!input.trim()) return; setUiError(null); clearPreparedRestart(); setPendingClarification(null); setReadyScenarioData(null); draftRecovery.submit(input, answers ?? clarifications, force) }, [clarifications, customInputZh, draftRecovery, setUiError])
  ...
  const resetCustom = useCallback(() => { clearPreparedRestart(); draftRecovery.discard(); setClarifications([]); setPendingClarification(null); setReadyScenarioData(null) }, [draftRecovery])
  const discardDraft = useCallback(() => { clearPreparedRestart(); draftRecovery.discard() }, [draftRecovery])
