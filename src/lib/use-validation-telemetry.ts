/**
 * [INPUT]: 既有会话/回合/复盘事实、内存中的服务端遥测凭据与 shared 严格 wire schema
...
import { getOrCreateValidationClientId } from './validation-client'
import { enqueueValidationRecord, flushValidationOutbox } from './validation-outbox'

export interface ValidationTelemetrySession { sessionId: string; telemetryToken: string }
interface TelemetryOptions { telemetrySession: ValidationTelemetrySession | null }
...
export function useValidationTelemetry({ telemetrySession }: TelemetryOptions) {
  ...
  const flush = useCallback(() => {
    ...
    void flushValidationOutbox(async (entry) => {
      ...
      const batch: ValidationBatch = ValidationBatchSchema.parse({ schemaVersion: VALIDATION_SCHEMA_VERSION, batchId: entry.batchId, sessionId: entry.sessionId, telemetryToken: session.telemetryToken, anonymousClientId, anonymousMetricsCollection: true, sentAt: entry.sentAt, records: [entry.record] })
      await sendValidationBatch(batch)
    }, session.sessionId).catch(() => undefined)
  }, [])
  useEffect(() => {
    const retry = () => flush()
    window.addEventListener('online', retry); window.addEventListener('focus', retry); flush()
  ...
  const enqueue = useCallback((dedupeKey: string, record: ValidationRecord) => {
    const session = sessionRef.current
    if (!session || seenRef.current.has(dedupeKey)) return
    const anonymousClientId = getOrCreateValidationClientId()
    if (!anonymousClientId) return
    const batchId = crypto.randomUUID(); const sentAt = Date.now()
    try { ValidationBatchSchema.parse({ schemaVersion: VALIDATION_SCHEMA_VERSION, batchId, sessionId: session.sessionId, telemetryToken: session.telemetryToken, anonymousClientId, anonymousMetricsCollection: true, sentAt, records: [record] }) } catch { return }
    seenRef.current.add(dedupeKey)
    void enqueueValidationRecord(batchId, session.sessionId, sentAt, record).then(() => flush()).catch(() => undefined)
  }, [flush])
  ...
  return useMemo(() => ({ checkpoint, roundSnapshot, sessionSummary, reviewResult, redoReview, flush }), [checkpoint, flush, redoReview, reviewResult, roundSnapshot, sessionSummary])
}
