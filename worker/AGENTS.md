...
> L2 | 父级: /AGENTS.md
成员清单
index.ts: Cloudflare Worker 入口与 API 路由编排，统一同源校验、请求解析、模型调用和响应边界，包括 POST /api/listening-scaffold 和 /api/practice/restart
...
tokens.ts: 场景、会话和独立长期练习凭据的 HMAC 签发、验签、过期和载荷边界
