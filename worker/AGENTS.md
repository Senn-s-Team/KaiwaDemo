...
> L2 | 父级: /AGENTS.md
成员清单
index.ts: Cloudflare Worker API entry, orchestrating training routes while remaining importable in Node tests
...
tokens.ts: 场景、会话、长期复练、短期任务 capability 的 HMAC 签发、验签、过期和域隔离边界
constants.ts: 五回合预算、请求限额、模型默认值与全局相手安全约束
env.ts: Worker secrets、模型、Mock 开关与 Workflow binding 类型
